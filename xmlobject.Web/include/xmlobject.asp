...
------------------------------------------------------
	...
	chriztian@steinmeier.dk - https://greystate.dk
	==================================================
	https://xmlobject.greystate.dk/
------------------------------------------------------

[Version 2.8.5]
	2002-10-19 00:45
		Separate caching of XML or XSL using bitflags.
		Bugfix in "loadServerXMLHTTP()" (trigged when
			using caching)
		New optional argument for "appendToRoot()":
			sXPath in appended document.
		New property: ".useXPath"
...
function XMLObject() {
	...
	if (this.useXPath) {
		if (this.input) {
			this.input.setProperty("SelectionLanguage", "XPath");
		}
	}
...
// ***********************************************************************************
	...
	function m_versionChanged() {
		switch (this.version) {
			case "default":
				...
				PROGID_SERVERXMLHTTP	= DEF_SERVERXMLHTTP;
			break;
			case "3.0":
				...
				PROGID_SERVERXMLHTTP	= PROGID_SERVERXMLHTTP_30;
			break;
			case "4.0":
				...
				PROGID_SERVERXMLHTTP	= PROGID_SERVERXMLHTTP_40;
			break;
	...
	function m_loadProcessorDocs(a_Names) {
		...
		var bReloadXML = (a_Names.length > 2 && (a_Names[2] & XMLOBJ_NOCACHE_XML)) ? true : false;
		var bReloadXSL = (a_Names.length > 2 && (a_Names[2] & XMLOBJ_NOCACHE_XSL)) ? true : false;
		...
		//	Handle XML document
        if ( typeof(Application(IDcachedXML)) == "undefined" || bForceReload || bReloadXML) {
			this.status = XMLOBJSTATE_LOADING;		// Signal loading in progress...
        ...
        if (typeof(Application(IDcachedXSL)) == "undefined" || bForceReload || bReloadXSL) {
			this.status = XMLOBJSTATE_LOADING;		// Signal loading in progress...
	...
	function loadDocument(xXMLspec, bAsyncLoad, bValid8OnParse, sCustomProgID) {
		...
		if (xXMLspec.indexOf("http://") == 0 || xXMLspec.indexOf(".asp") > 1) {	// Must use ServerXMLHTTP
			return loadServerXMLHTTP(xXMLspec, bAsyncLoad, bValid8OnParse);
		}
		...
		if (xXMLspec.indexOf("file://") == 0)	// URL
			oTemp.load(xXMLspec);
	...
	function loadServerXMLHTTP(sURL, bAsync, bValidate) {
        var oTemp = Server.CreateObject(PROGID_SERVERXMLHTTP);
		var oReturn = Server.CreateObject(PROGID_FREETHREADED);

		oTemp.open("GET", sURL, bAsync);
		oTemp.send();

		oReturn.load(oTemp.responseXML);
		return oReturn;
    }
...
//		this.output += this.getCacheInfo();		// For debugging purposes...
	}
	...
	function m_replaceNode(xXMLnode, sXPath) {
		if (xXMLnode && sXPath) {
			...
			if (oOldNode) {
				...
				throwError(XMLOBJERROR_NODE_NOT_FOUND, "The XPath expression &quot;<b>" + sXPath + "</b>&quot; did not return an XML node.");
		} else
			throwError(XMLOBJERROR_INVALID_ARGUMENT);
	}

	// Append a new node to the root of the XML document.
	function m_appendToRoot(xXMLnode/* Optional */, sXPath) {
		if (xXMLnode) {
			var oNewNode, oNewNodeDoc = loadDocument(xXMLnode, false, false, this.progID);
			if (sXPath) {
				if (this.useXPath) {
					oNewNodeDoc.setProperty("SelectionLanguage", "XPath");
				}
				oNewNode = oNewNodeDoc.selectSingleNode(sXPath);
				if (!oNewNode) {
					throwError(XMLOBJERROR_NODE_NOT_FOUND, "The XPath expression &quot;<b>" + sXPath + "</b>&quot; did not return an XML node.");
				}
			} else {
				oNewNode = oNewNodeDoc.documentElement;
			}

			this.input.documentElement.appendChild(oNewNode);
			this.xml = this.input.xml;								// Update .xml property.
		} else
			throwError(XMLOBJERROR_INVALID_ARGUMENT);
	}

	function m_appendToNode(xXMLnode, sXPath) {
        if (xXMLnode && sXPath) {
			var oOldNode = this.input.selectSingleNode(sXPath);
			if (oOldNode) {
				var oNewNode = loadDocument(xXMLnode, false, false, this.progID);
				oOldNode.appendChild(oNewNode.documentElement);
				this.xml = this.input.xml;								// Update .xml property.
			} else
				throwError(XMLOBJERROR_NODE_NOT_FOUND, "The XPath expression &quot;<b>" + sXPath + "</b>&quot; did not return an XML node.");
		}
